...
static void push_to_queue(NodeQueue queues[], float queue_first_wait_time[], int node, int traveler_id, float now) {
if (queues[node].count == 0) {
queue_first_wait_time[node] = now;
}

    
queue_push(&queues[node], traveler_id);
...
float queue_first_wait_time[MAX_NODES];

for (int i = 0; i < g.num_nodes; i++) {
    queue_init(&queues[i]);
    occupant[i] = -1;
    queue_first_wait_time[i] = 0.0f;
}
...
for (int i = 0; i < num_travelers; i++) {
    ...
    if (travelers[i].path_count > 0) {
        push_to_queue(queues, queue_first_wait_time, travelers[i].current_node, i, 0.0f);
    } else {
...
while (!WindowShouldClose()) {
    ...
    for (int node = 0; node < g.num_nodes; node++) {
        if (occupant[node] == -1 && !queue_is_empty(&queues[node])) {
            ...
            if (scheduler == SCHD_SJF && queues[node].count == 1) {
                if (now - queue_first_wait_time[node] < SJF_BATCH_TIME) {
                    can_schedule = 0;
                }
            }
            ...
            if (can_schedule) {
                ...
                if (chosen != -1) {
                    ...
                    if (queues[node].count > 0) {
                        ...
                        queue_first_wait_time[node] = 0.0f;
                    }
    ...
    for (int i = 0; i < num_travelers; i++) {
        if (travelers[i].status == IN_NODE) {
            ...
            if (progress >= 1.0f) {
                ...
                push_to_queue(queues, queue_first_wait_time, v, i, now);
    ...
    if (scheduler == SCHD_SJF) {
        DrawText(
            "SJF waits shortly to collect waiting travelers, then chooses shortest remaining path",
            20,
            82,
            16,
            DARKGRAY
