...
static void draw_arrow(Vector2 start, Vector2 end) {
    ...
    Vector2 p1 = { end.x - size * cosf(angle - PI / 6), end.y - size * sinf(angle - PI / 6) };
    Vector2 p2 = { end.x - size * cosf(angle + PI / 6), end.y - size * sinf(angle + PI / 6) };
...
static void draw_travelers(TravelerState travelers[], int count, Vector2 node_pos[]) {
    for (int i = 0; i < count; i++) {
        Vector2 p = travelers[i].status == MOVING ? travelers[i].pos : node_pos[travelers[i].current_node];
        ...
        DrawCircleV(p, 13, c);
        DrawText(TextFormat("T%d", i + 1), (int)p.x - 10, (int)p.y - 30, 14, BLACK);
    }
}

static void build_scheduler_view(TravelerState travelers[], SchedulerTravelerView view[], int count) {
    for (int i = 0; i < count; i++) {
        view[i].path_count = travelers[i].path_count;
        view[i].path_index = travelers[i].path_index;
...
int main(int argc, char* argv[]) {
    ...
    while (!WindowShouldClose()) {
        ...
        for (int node = 0; node < g.num_nodes; node++) {
            if (occupant[node] == -1 && !queue_is_empty(&queues[node])) {
                int chosen = choose_traveler_from_queue(&queues[node], scheduler, view, &g);
